...
#define SYSPLLCTRL_Val        0x00000020  // (0+1) * XTAL => 12 MHz
#endif
#if (LPC_CORE_CLOCKSPEED_HZ == 24000000)
#define SYSPLLCTRL_Val        0x00000021  // (1+1) * XTAL => 24 MHz
#endif
#if (LPC_CORE_CLOCKSPEED_HZ == 36000000)
#define SYSPLLCTRL_Val        0x00000022  // (2+1) * XTAL => 36 MHz
#endif
#if (LPC_CORE_CLOCKSPEED_HZ == 48000000)
#define SYSPLLCTRL_Val        0x00000023  // (3+1) * XTAL => 48 MHz
#endif
#if (LPC_CORE_CLOCKSPEED_HZ == 60000000)
#define SYSPLLCTRL_Val        0x00000024  // (4+1) * XTAL => 60 MHz
#endif
#if (LPC_CORE_CLOCKSPEED_HZ == 72000000)
#define SYSPLLCTRL_Val        0x00000025  // (5+1) * XTAL => 72 MHz
#endif
...
{
  return __WDT_OSC_CLK; // Watch out: this is related to the above function
}
...
void SystemCoreClockUpdate (void)            /* Get Core Clock Frequency      */
{
  ...
  wdt_osc /= ((SYSCON->WDTOSCCTRL & 0x1F) << 1) + 2;
  ...
  switch (SYSCON->MAINCLKSEL & 0x03) {
    ...
    case 3:                             /* System PLL Clock Out               */
      switch (SYSCON->SYSPLLCLKSEL & 0x03) {
          case 0:                       /* Internal RC oscillator             */
            if (SYSCON->SYSPLLCTRL & 0x180) {
              SystemCoreClock = __IRC_OSC_CLK;
            } else {
              SystemCoreClock = __IRC_OSC_CLK * ((SYSCON->SYSPLLCTRL & 0x01F) + 1);
            }
            break;
          case 1:                       /* System oscillator                  */
            if (SYSCON->SYSPLLCTRL & 0x180) {
              ...
              SystemCoreClock = __SYS_OSC_CLK * ((SYSCON->SYSPLLCTRL & 0x01F) + 1);
            }
            break;
          case 2:                       /* WDT Oscillator                     */
            if (SYSCON->SYSPLLCTRL & 0x180) {
...
void SystemInit (void)
{
  ...
  SYSCON->PDRUNCFG     &= ~(1 << 5);          /* Power-up System Osc      */
  SYSCON->SYSOSCCTRL    = SYSOSCCTRL_Val;
  for (i = 0; i < 200; i++) __NOP();
  SYSCON->SYSPLLCLKSEL  = SYSPLLCLKSEL_Val;   /* Select PLL Input         */
  SYSCON->SYSPLLCLKUEN  = 0x01;               /* Update Clock Source      */
  SYSCON->SYSPLLCLKUEN  = 0x00;               /* Toggle Update Register   */
  SYSCON->SYSPLLCLKUEN  = 0x01;
  while (!(SYSCON->SYSPLLCLKUEN & 0x01));     /* Wait Until Updated       */
#if (SYSPLL_SETUP)                                /* System PLL Setup         */
  SYSCON->SYSPLLCTRL    = SYSPLLCTRL_Val;
  SYSCON->PDRUNCFG     &= ~(1 << 7);          /* Power-up SYSPLL          */
  while (!(SYSCON->SYSPLLSTAT & 0x01));	      /* Wait Until PLL Locked    */
#endif
#endif
#if (WDTOSC_SETUP)                                /* Watchdog Oscillator Setup*/
  SYSCON->WDTOSCCTRL    = WDTOSCCTRL_Val;
  SYSCON->PDRUNCFG     &= ~(1 << 6);          /* Power-up WDT Clock       */
#endif
  SYSCON->MAINCLKSEL    = MAINCLKSEL_Val;     /* Select PLL Clock Output  */
  SYSCON->MAINCLKUEN    = 0x01;               /* Update MCLK Clock Source */
  SYSCON->MAINCLKUEN    = 0x00;               /* Toggle Update Register   */
  SYSCON->MAINCLKUEN    = 0x01;
  while (!(SYSCON->MAINCLKUEN & 0x01));       /* Wait Until Updated       */
#endif

#if (USBCLK_SETUP)                                /* USB Clock Setup          */
  SYSCON->PDRUNCFG     &= ~(1 << 10);         /* Power-up USB PHY         */
#if (USBPLL_SETUP)                                /* USB PLL Setup            */
  SYSCON->PDRUNCFG     &= ~(1 <<  8);         /* Power-up USB PLL         */
  SYSCON->USBPLLCLKSEL  = USBPLLCLKSEL_Val;   /* Select PLL Input         */
  SYSCON->USBPLLCLKUEN  = 0x01;               /* Update Clock Source      */
  SYSCON->USBPLLCLKUEN  = 0x00;               /* Toggle Update Register   */
  SYSCON->USBPLLCLKUEN  = 0x01;
  while (!(SYSCON->USBPLLCLKUEN & 0x01));     /* Wait Until Updated       */
  SYSCON->USBPLLCTRL    = USBPLLCTRL_Val;
  while (!(SYSCON->USBPLLSTAT   & 0x01));     /* Wait Until PLL Locked    */
  SYSCON->USBCLKSEL     = 0x00;               /* Select USB PLL           */
#else
  SYSCON->USBCLKSEL     = 0x01;               /* Select Main Clock        */
#endif
#else
  SYSCON->PDRUNCFG     |=  (1 << 10);         /* Power-down USB PHY       */
  SYSCON->PDRUNCFG     |=  (1 <<  8);         /* Power-down USB PLL       */
#endif

  SYSCON->SYSAHBCLKDIV  = SYSAHBCLKDIV_Val;
  SYSCON->SYSAHBCLKCTRL = AHBCLKCTRL_Val;
#endif


#if (MEMMAP_SETUP || MEMMAP_INIT)       /* Memory Mapping Setup               */
  _SYSCON->SYSMEMREMAP = SYSMEMREMAP_Val;
#endif
}
