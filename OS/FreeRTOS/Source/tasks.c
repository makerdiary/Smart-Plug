...
// JvdG: Added next 3 lines
#if ( configGENERATE_RUN_TIME_STATS == 1 )
portCHAR volatile cResetRunTimeCounter = 0;
#endif
...
/*----------------------------------------------------------*/
	...
	unsigned portLONG ulTotalRunTime = portGET_RUN_TIME_COUNTER_VALUE();
        ...
        vTaskSuspendAll();
		{
            // JvdG: Added next 5 lines
            if (cResetRunTimeCounter != 0)
            {
                portCONFIGURE_TIMER_FOR_RUN_TIME_STATS(); // reset the counter
			...
			pcWriteBuffer[ 0 ] = ( signed portCHAR ) 0x00;
// JvdG: Commented out the next line
//			strcat( ( portCHAR * ) pcWriteBuffer, ( const portCHAR * ) "\r\n" );
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				else
				{
                    ...
                    // JvdG: Added next line
                    if (cResetRunTimeCounter != 0) pxNextTCB->ulRunTimeCounter = 0;
				}
