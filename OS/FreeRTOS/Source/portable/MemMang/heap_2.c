...
static int totalinuse = 0;    /* PN / JvdG */
...
void *pvReturn = NULL;
    ...
    /* PN / JvdG */
	if (pvReturn) {
        unsigned portCHAR *puc = (( unsigned portCHAR * ) pvReturn) - heapSTRUCT_SIZE;
        xBlockLink *pxLink = ( void * ) puc;
		totalinuse += pxLink->xBlockSize;
	}
...
xBlockLink *pxLink;
	...
	if( pv )
	{
		...
		vTaskSuspendAll();
		{				
			totalinuse -= pxLink->xBlockSize;      /* PN / JvdG */
